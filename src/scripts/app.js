...
const API_BASE_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
    ...
    : 'https://wemarket4u-production.up.railway.app'; // Production - Railway backend
...
function initTryForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (extra) send.append('additional_prompt', extra);
            ...
            try {
                const res = await fetch(apiUrl('/api/generate-image'), { method: 'POST', body: send });
                let data = {};
                try {
                    data = await res.json();
                } catch (parseErr) {
...
function initHistoryLookup() {
    ...
    historyForm.addEventListener('submit', async (e) => {
        ...
        historyResults.innerHTML = '<p>Loading your history...</p>';
        historyResults.style.display = 'block';
        
        try {
            const res = await fetch(apiUrl(`/api/history/${encodeURIComponent(email)}`));
            const data = await res.json();
            
            if (!res.ok) {
                historyResults.innerHTML = `<p style="color:#dc2626;">${data.error || 'Failed to load history'}</p>`;
                return;
            }
            
            if (!data.generations || data.generations.length === 0) {
                historyResults.innerHTML = '<p>No generations found for this email. Try creating one above!</p>';
                return;
            }
            
            let html = `<p>Found <strong>${data.total}</strong> generation(s) for ${email}</p>`;
            html += '<div class="history-grid">';
            
            data.generations.forEach((gen, idx) => {
                const date = new Date(gen.created_at).toLocaleDateString();
                const status = gen.status === 'completed' ? '✅' : gen.status === 'pending' ? '⏳' : '❌';
                html += `
                    <div class="history-card">
                        <div class="history-card-header">
                            <span>${status} Generation #${data.total - idx}</span>
                            <small>${date}</small>
                        </div>
                        ${gen.output_image_path ? 
                            `<img src="${gen.output_image_path}" alt="Generated image" class="history-img" loading="lazy" />` 
                            : '<div class="history-img-placeholder">No image</div>'
                        }
                        <p class="history-prompt">"${gen.prompt?.substring(0, 80)}${gen.prompt?.length > 80 ? '...' : ''}"</p>
                        ${gen.output_image_path ? 
                            `<a href="${gen.output_image_path}" download class="button small">Download</a>` 
                            : ''
                        }
                    </div>
                `;
            ...
            historyResults.innerHTML = `<p style="color:#dc2626;">Error: ${err.message}</p>`;
        }
    });
}
